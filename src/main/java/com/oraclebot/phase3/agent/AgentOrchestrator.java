...
public class AgentOrchestrator {
    ...
    public String handleMessage(String messageText) {
        if (messageText.trim().equalsIgnoreCase("debug")) {
            ParsedIntent probe = llmIntentParser.parse("lista todas las tareas");
            return "[DEBUG]\nAI enabled: " + aiProps.isEnabled()
                    + "\nModel: " + aiProps.getModel()
                    + "\nBase URL: " + aiProps.getBaseUrl()
                    + "\nProbe intent (lista todas las tareas): " + probe.getIntent();
        }
        ...
        return switch (parsedIntent.getIntent()) {
            ...
            case UNKNOWN -> llmIntentParser.askGeneral(messageText);
        };
    }
    ...
    private String createTask(ParsedIntent parsedIntent) {
        ...
        return """
                ...
                Story points: %d
                Sprint: %s
                """.formatted(task.getId(), task.getTitle(), task.getAssignee(), task.getStatus(),
                task.getStoryPoints(), task.getSprintName()).trim();
    }
    ...
    private String sprintSummary() {
        ...
        return """
                ...
                """.formatted(sprint.getName(), sprint.getStartDate(), sprint.getEndDate(), sprintTasks.size(), done,
                inProgress, pending, totalPoints).trim();
    }
